...
public class DownloadAllIssueCommentsInLocalFolderMono : MonoBehaviour
{
    ...
    public IEnumerator CoroutineLoadAll()
    {
        ...
        foreach (var item in m_userRepository)
        {
            ...
            for (int i = 1; i < m_maxIndex && continueSearch; i++)
            {
                ...
                GitHubStoragePathUtility.GetIssueCommentsPathJson(
                ...
                yield return GitHubFetchJsonTool.FetchIssueComments
                    (   item, item, index, m_apiKey, m_lastJsonDownloaded);
